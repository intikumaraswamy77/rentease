...
const publicDir = 'e:/unified mentors 2/rentease-frontend/public/images';
...
mongoose.connect('mongodb://127.0.0.1:27017/rentease').then(async () => {
  ...
  for(let i = 0; i < products.length; i++) {
    ...
    try {
      execSync(`curl -sL "${p.url}" -o "${filePath}"`);
      // Update DB to use local path
      await Product.updateMany({ name: p.name }, { $set: { image: '/images/' + filename } });
      console.log('Downloaded and updated: ' + p.name);
    } catch (e) {
      console.log('Failed to download: ' + p.name);
    }
  }
